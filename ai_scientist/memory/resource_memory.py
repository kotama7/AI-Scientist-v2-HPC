...
def persist_resource_snapshot_to_ltm(
    snapshot: ResourceSnapshot,
    ltm: Any,
    *,
    ...
    use_branch = _ltm_branch_id(ltm, branch_id)
    if not use_branch:
        raise ValueError("Missing branch_id for resource snapshot persistence")
    index_max = int(getattr(ltm, "core_max_chars", RESOURCE_INDEX_MAX_CHARS_DEFAULT))
    index_text = _format_resource_index(
        snapshot,
    ...
    if previous_digest:
        index_json["previous_digest"] = previous_digest
    if hasattr(ltm, "mem_core_set") and getattr(ltm, "root_branch_id", None):
        ltm.mem_core_set(
    ...
    for item in snapshot.items:
        has_payload = bool(item.tree or item.files)
        ...
        for key in sorted(header.keys()):
            header_lines.append(f"{key}: {_safe_json(header[key])}")
        header_lines.append("---")
        ...
        if hasattr(ltm, "mem_resources_snapshot_upsert"):
            try:
                ltm.mem_resources_snapshot_upsert(
                    str(run_id or ""),
                    item.name,
                    _item_digest_payload(item),
                )
            except Exception:
                pass
        ltm.write_archival(use_branch, text, tags=tags)
    ...
    try:
        if hasattr(ltm, "mem_recall_append"):
            ...
            ltm.write_event(
                use_branch,
                "resource_index_updated",
                f"digest={snapshot.resource_digest}",
                tags=["RESOURCE_INDEX", f"resource_digest:{snapshot.resource_digest}"],
            )
    except Exception as exc:
        logger.warning("Failed to write resource index event: %s", exc)
    _write_snapshot_files(snapshot, ltm)


def update_resource_snapshot_if_changed(
    snapshot: ResourceSnapshot,
    ltm: Any,
    *,
    branch_id: str | None = None,
    ...
    use_branch = _ltm_branch_id(ltm, branch_id)
    if not use_branch:
        raise ValueError("Missing branch_id for resource snapshot update")
    previous_digest = ltm.get_core(use_branch, RESOURCE_DIGEST_KEY)
    if previous_digest == snapshot.resource_digest:
        return
