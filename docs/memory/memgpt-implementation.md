...
def generate_final_memory_for_paper(
    ...
    """
    Generate final memory for paper generation.

    Args:
        run_dir: Run directory path
        root_branch_id: Root branch ID
        best_branch_id: Best branch ID (or None to use root)
        artifacts_index: Optional artifacts index (best/top nodes)
        no_budget_limit: If True, do not truncate/compress content

    Returns:
        Dictionary with all paper sections
    """
    # Load resource snapshot/index (if present)
    # Build sections via _build_paper_sections()
    # Attach artifacts_index if provided
    # Write final_memory_for_paper.md under run_dir/memory/
    # Return sections dict (caller may serialize if needed)
```

**Note**: The current implementation writes only the markdown file
(`final_memory_for_paper.md`). A structured JSON export is not generated by
default.
