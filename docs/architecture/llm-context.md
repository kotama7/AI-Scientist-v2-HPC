...
Prompt templates are stored under `prompt/` and assembled per stage. The directory
structure is organized as follows:

```
prompt/
├── core/                # Shared prompt components
│   ├── system.txt       # Global system instructions
│   ├── domain_neutral.txt  # Domain-neutral guidance
...
│   ├── manager/         # Agent manager stage prompts
...
The most commonly edited files include:

- `prompt/core/system.txt` (global system instructions).
- `prompt/agent/parallel/` (stage introductions and response formats).
- `prompt/output/writeup/` (writeup pipeline).
- `prompt/review/llm/` (paper review prompts).

## Persona system

The persona system (`ai_scientist/persona.py`) allows customization of the
agent's role in prompts. Configure via `agent.role_description` in
`bfts_config.yaml`:
...
- **Introduction**: `prompt/config/phases/phase0_planning.txt` (schema and constraints)
...
- **Introduction**: `prompt/config/phases/phase1_installer.txt` (rules and command categories)
- **Task**: Research idea/experiment description
...
JSON following `prompt/agent/parallel/response_format/execution_split.txt` with:
...
`prompt/agent/parallel/response_format/execution_split.txt`.
...
- Plotting: Response format + plotting guideline (experiment code + optional
  base plotting code) + Memory.
- Plot selection: Introduction + plot paths + Memory.
- VLM analysis: research idea text + selected plot images (base64) + Memory.
...
- Parse-metrics plan: original code + prior parse errors/code + instructions +
  example parser + response format + Memory.
- Metric extraction: parser execution output + Memory.

## Node summary

- Introduction + research idea + implementation + plan + execution output +
  analysis + metric + plot analyses + VLM feedback + Memory.
