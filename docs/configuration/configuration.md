...
- `use_fts`: full-text search mode (`auto`, `on`, or `off`). Use `off` to disable; use `on` to require FTS.

Persistence and exports:

- `final_memory_enabled`: write `final_memory_for_paper.*` at run end.
- `final_memory_filename_md`: output markdown filename (JSON output is not generated by default).
